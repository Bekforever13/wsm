export const qq = {
	translation: {
		// navbar
		home: 'Бас бет',
		transactions: 'Транзакция',
		products: 'Өнимлер',
		categories: 'Тайпалар',
		brands: 'Брендлар',
		storage: 'Склад',
		users: 'Пайдаланыўшылар',

		// header
		language: 'Тил',

		// home page

		// transactions page
		transactionsTableCol1: 'Өним аты',
		transactionsTableCol2: 'Транзакция түри',
		transactionsTableCol3: 'Төлеў',
		transactionsTableCol4: 'Баҳасы',
		transactionsTableCol5: 'Муғдары',
		transactionsTableCol6: 'Дата',
		transactionsMessageRequired: 'Транзакция атын жазың!',
		transactionsMessageCreateSuccess: 'Транзакция жаратылды!',
		transactionsMessageEditSuccess: 'Транзакция өзгертилди!',
		transactionsMessageDeleteSuccess: 'Транзакция өширилди!',
		transactionsFormProducts: '',
		transactionsFormPayment: '',
		transactionsFormTransactions: '',
		transactionsFormPrice: '',
		transactionsFormQuantity: '',
		transactionsFormDate: '',

		// products page
		productsTableCol1: 'Өним аты',
		productsTableCol2: 'Тайпа',
		productsTableCol3: 'Бренд',

		// categories page
		categoriesTableCol1: 'Тайпа аты',
		newCategory: 'Таза тайпа',
		categoryMessageRequired: 'Тайпаның атын жазың!',
		categoryMessageCreateSuccess: 'Тайпа жаратылды!',
		categoryMessageEditSuccess: 'Тайпа өзгертилди!',
		categoryMessageDeleteSuccess: 'Тайпа өширилди!',

		// brands page
		brandsTableCol1: 'Бренд аты',
		newBrands: 'Таза бренд',
		brandsMessageRequired: 'Бренд атын жазың!',
		brandsMessageCreateSuccess: 'Бренд жаратылды!',
		brandsMessageEditSuccess: 'Бренд өзгертилди!',
		brandsMessageDeleteSuccess: 'Бренд өширилди!',

		// storage page
		storageTableCol1: 'Өним аты',
		storageTableCol2: 'Тайпа',
		storageTableCol3: 'Бренд',
		storageTableCol4: 'Сатып алынған',
		storageTableCol5: 'Сатылған',
		storageTableCol6: 'Қалғаны',

		// users page
		userTableCol1: 'Ф.И.О',
		userTableCol2: 'Телефон номери',
		userTableCol3: 'Роль',

		// shared
		add: 'Қосыў',
		edit: 'Өзгертиў',
		delete: 'Өшириў',
		save: 'Сақлаў',
		cancel: 'Быйкарлаў',
		actions: 'Ҳәрекетлер',
		exit: 'Шығыу',
		beforeExit: 'Ҳақыйқаттан да кетиўди қәлейсизбе?',
		beforeDelete: 'Ҳақыйқаттан да өшириўди қәлейсизбе?',
		select: 'Сайлаң',
		name: 'Аты',
		cash: 'Нақ пул',
		plastic_card: 'Пластик карта',
		credit: 'Кредит',
		purchased: 'Сатып алынды',
		sales: 'Сатылды',
	},
}
